...
public class MainnetTransactionProcessor implements TransactionProcessor {
    ...
    public Result processTransaction(
        ...
        final Wei gasPrice;
        if (transaction.isEIP1559Transaction() && blockHeader instanceof BlockHeader) {
            gasPrice = transaction.computeGasPriceEIP1559(((BlockHeader) blockHeader).getBaseFee().orElseThrow());
        } else {
        ...
        if (transaction.isEIP1559Transaction() && blockHeader instanceof BlockHeader) {
            coinbaseWei = coinbaseFee.priceFor(Wei.of(gasPrice.toLong() - ((BlockHeader) blockHeader).getBaseFee().orElseThrow()));
        } else {
            coinbaseWei = coinbaseFee.priceFor(gasPrice);
        }
